...
-- https://github.com/pysal/giddy/blob/master/tools/github_stats.py
...
def parse_link_header(headers):
    """
    Return dictionary of parsed headers
    """
...
def get_issues(project="pysal/"+PACKAGE, state="closed", pulls=False):
    """
    Get a list of the issues from the Github API.
    """
    which = 'pulls' if pulls else 'issues'
    url_str = "https://api.github.com/repos/%s/%s?state=%s&per_page=%i"
    url = url_str % (project, which, state, PER_PAGE)
...
def issues_closed_since(period=timedelta(days=365),
                        project="pysal/"+PACKAGE, pulls=False):
    """
    ...
    url_str = "https://api.github.com/repos/%s/%s?state=closed"\
              + "&sort=updated&since=%s&per_page=%i"
    url = url_str % (project, which, period.strftime(ISO8601), PER_PAGE)
